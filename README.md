...
| 指标 | 已知类型 A~E | 冷启动 G | 随机预测 | 判定 |
|------|:----:|:----:|:----:|:----:|
| ACC@1 | 0.551 | 0.121 | <0.05 | ✅ 超随机 |
| ACC@3 | 0.913 | 0.443 | <0.15 | ✅ 超随机 |
| ACC@5 | 0.980 | 0.702 | <0.25 | ✅ 超随机 |
| Root AUC | 0.429 | — | 0.50 | — |

冷启动类型 F 全部为独立根因（无 derivative 事件），ContextEmbedder 将其全部正确判定为根因，未出现误分类。冷启动类型 G 的因果定位指标在零训练的前提下显著高于随机预测，证明 $h(t^-) \to v$ 映射能够从未见类型的时序上下文中提取有效的因果信息。
