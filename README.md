...
**流程**：全局前 3 次遇到某冷启动类型时做 5 步梯度下降（充分探索不同上下文），后续该类型再做 1 步热启动更新。微调仅作用于新类型的嵌入向量 $v$，ODE-RNN、decoder 及其他类型嵌入全部冻结。事件的发生本身即为自监督信号，无需额外标注。
...
前 3 次 $S=5$ 步（冷启动收敛），后续 $S=1$ 步（热启动微调）。$\bar{v}$ 为已知类型嵌入的均值，$\alpha=0.01$ 防止 $v$ 偏离太远导致全判 Derivative。
