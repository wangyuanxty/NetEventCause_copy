...
## 2. 方案设计：ContextEmbedder

### 核心思想

NEC 的事件嵌入 $v_k$ 与时序隐藏状态 $h(t^-)$ 之间仅有查表关系。$h(t_i^-)$ 仅由前序事件决定，与当前类型无关——**新类型首次出现时 $h(t_i^-)$ 已包含推断其行为所需的全部时序信息**。解决冷启动归结为构建可训练映射 $f_\theta: h(t^-) \to v$，且训练阶段不依赖新类型标注。

方案是在 NEC 中插入轻量 MLP `ContextEmbedder`：训练时随机 mask 15% 已知类型，被 mask 事件以 $f_\theta(h(t^-))$ 替代查表；推理时对未见类型自动触发。
...
训练和推理阶段 ContextEmbedder 调用完全一致——每次出现均从当前 $h(t^-)$ 重新生成嵌入，不缓存。仅在类型选择上不同：训练 mask 已知类型，推理检测未见类型。
