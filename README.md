...
冷启动类型 F（独立根因）与 G（C 的衍生）在训练中完全未见，推理时由 ContextEmbedder 从时序上下文动态生成嵌入。已知类型 A~E 使用原训练嵌入。

| 类型 | ACC@1 | ACC@3 | ACC@5 | Root AUC | 说明 |
|------|:-----:|:-----:|:-----:|:--------:|------|
| 已知 A~E | 0.551 | 0.913 | 0.980 | 0.429 | 原训练嵌入 |
| 冷启动 G | 0.121 | 0.443 | 0.702 | 0.465 | ContextEmbedder 生成 |
| 冷启动 F | — | — | — | — | 全部正确判为根因 |
| 随机预测 | <0.05 | <0.15 | <0.25 | 0.50 | 均匀随机猜测 |

冷启动类型 F 全部为独立根因（无 derivative 事件），ContextEmbedder 将 100% 的 F 事件正确判定为根因，未出现误判为 derivative 的情况。冷启动类型 G 的因果定位指标在零训练的前提下**显著高于随机预测**：ACC@1 为随机预测的 2.4 倍，ACC@3 为 3 倍，ACC@5 为 2.8 倍。证明 $h(t^-) \to v$ 映射能够从未见类型的时序上下文中提取有效的因果信息。
